...
jQuery(function($){
    ...
    // ANIMATE
    $('.entry-fadeIn > *:not(:is(ul,ol,table)), .entry-fadeIn *:is(li,tr), .entry-fadeIn .wp-block-column > *').addClass('animate fadeIn');
    $('.entry-fadeIn .wp-block-image img').addClass('animate matrixZoomOut');

    $('.entry-fadeInUp > *:not(:is(ul,ol)), .entry-fadeIn *:is(li), .entry-fadeIn .wp-block-column > *').addClass('animate fadeInUp');
    $('.entry-fadeInUp .wp-block-image img').addClass('animate matrixZoomOut');

    $('.accordion .animate').each(function(){ $(this).removeClass('animate fadeIn'); });
...
jQuery(function($){
    ...
    function showTab(elem){
        ...
        if($(elem).parents('.tab-container').hasClass('offset-sction')) {
            $("html,body").animate({scrollTop: $(elem).closest('section').offset().top - _windowHeight }, 800);
        } else {
            $("html,body").animate({scrollTop: $(elem).offset().top - _windowHeight }, 800);
        }
    ...
    $('.tabdata-container').each(function(){
        ...
        function showTabData(elem){
            ...
            $("html,body").animate({scrollTop: tabDataGroupParent.find('.tabdata-content' + '*[data-tab-id=' + elem + ']').offset().top - _windowHeight }, 800);
        }
...
jQuery(function($){
        ...
        $('.swiper.default').each(function(){
            ...
            function switchColor() {
                ...
                if(_this.find('.sc-billboard > .sc-cover .swiper-slide').length) {
                    _this.find('.sc-billboard > .sc-cover .swiper-slide').each(function(){
                        var _slideParent = $(this).parents('.sc-cover');
                        var _slideParents = $(this).parents('.sc-billboard');

                        if(_slideParent.find('.swiper-slide-active').hasClass('c-white')) {
                            setTimeout(function(){
    ...
    if($('.swiper.auto').length){
        $('.swiper.auto').each(function(){
            ...
            if(_this.hasClass('fix')) {
                _this.closest('*[class*="-slider"]').find('*[class*="swiper-button-"]').css('height', _this.find('[class*="object"]').outerHeight()).addClass();
                $(window).resize(function(){
                    _this.closest('*[class*="-slider"]').find('*[class*="swiper-button-"]').css('height', _this.find('[class*="object"]').outerHeight());
                });
            }
...
jQuery(function($){
    ...
    $('.brand-container').each(function(){
        var _this = $(this);
            _this.find('> .brand-nav').clone().appendTo( _this ).addClass('marquee').wrapInner('<div class="brand-marquee"></div>');
